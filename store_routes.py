...
async def get_next_order_id():
    doc = await db.counters.find_one_and_update({"_id": "global_order_id"}, {"$inc": {"seq": 1}}, upsert=True, return_document=True)
    return doc["seq"]
...
# ==========================================
# المصادقة (دعم محفظتين: EGP و USD)
# ==========================================
...
# ==========================================
# الشراء والشحن بالعملتين (EGP و USD)
# ==========================================
...
async def customer_buy_uc(request: Request, stock_key: str = Form(...), price: float = Form(...), currency: str = Form(...)):
    ...
    # التحقق من الرصيد بناءً على العملة اللي اختارها
...
# ==========================================
# أدمن الاستور (CRM مخصص للعملاء والمبيعات فقط)
# ==========================================
