...
async def log_wallet_txn(
    email: str, amount: float, currency: str, note: str, ref: str = "", session=None
):
...
async def _get_product_price(stock_key: str, currency: str):
    cat = await db.store_categories.find_one(
        {"products.stock_key": stock_key}, {"products.$": 1}
    )
...
async def _purchase_one_unit(email: str, user_name: str, stock_key: str, currency: str):
    ...
    async def _run_with_session(session):
        code_doc = await db.stock.find_one_and_delete(
            {"category": stock_key}, session=session
        )
        if not code_doc:
        ...
        await log_wallet_txn(
            email, -price, currency, f"Order #{order_id}", ref=order_id, session=session
        )
        return {
            "success": True,
            "order_id": order_id,
    ...
    if bal_res.modified_count != 1:
        await db.stock.insert_one(
            {
                "code": code_doc.get("code") or str(code_doc.get("_id")),
                "category": stock_key,
...
async def login_manual(
    ...
    _set_store_session_cookie(response, request, user["email"])
    return response
...
async def signup_verify(
    ...
    if (datetime.now() - otp_doc["created_at"]).total_seconds() > 300:
        return JSONResponse(
            {"success": False, "msg": "Code expired! Please try again."}
        )
    ...
    _set_store_session_cookie(response, request, email)
    return response
...
async def forgot_password(request: Request, email: str = Form(...)):
    if not await db.store_customers.find_one({"email": email}):
        return JSONResponse(
            {"success": False, "msg": "Account with this email does not exist."}
        )
    ...
    return JSONResponse(
        {"success": True, "msg": "Password reset code sent to your email!"}
    )
...
async def customer_buy(
    ...
    if not txid:
        return JSONResponse(
            {
                "success": False,
                "msg": "Duplicate transaction detected. This payment was already processed.",
            }
        )

    try:
        result = await _purchase_one_unit(
            email, user.get("name", "Customer"), stock_key, currency
        )
        if not result.get("success"):
            await _finish_txn_lock(txid, "failed", result.get("msg", "failed"))
            return JSONResponse(
                {"success": False, "msg": result.get("msg", "Purchase failed")}
            )
        ...
        return JSONResponse(
            {"success": False, "msg": "Purchase failed due to a server error."}
        )
...
async def wallet_history(request: Request):
    ...
    txns = (
        await db.store_wallet_ledger.find({"email": email}).sort("ts", -1).to_list(200)
    )
    for t in txns:
        t["id"] = str(t.get("_id", ""))
        if "_id" in t:
...
async def checkout_cart(request: Request, payload: CheckoutRequest):
    ...
    if not txid:
        return JSONResponse(
            {
                "success": False,
                "msg": "Duplicate transaction detected. This checkout was already processed.",
            }
        )
    ...
    try:
        ...
        fresh_balances_source = fresh_user or {}
        new_balances = {
            key: value
            for key, value in fresh_balances_source.items()
        ...
        await _finish_txn_lock(txid, "failed", "internal_error")
        return JSONResponse(
            {"success": False, "msg": "Checkout failed due to server error."}
        )
...
async def store_manage_balance(
    ...
    sanitize_positive_int = globals().get("_sanitize_positive_int")
    normalize_currency = globals().get("_normalize_currency")
    acquire_txn_lock = globals().get("_acquire_txn_lock")
    finish_txn_lock = globals().get("_finish_txn_lock")

    if not callable(sanitize_positive_int) or not callable(normalize_currency):
        return JSONResponse(
            {"success": False, "msg": "Validation helpers are unavailable."}
        )
    if not callable(acquire_txn_lock) or not callable(finish_txn_lock):
        return JSONResponse(
            {"success": False, "msg": "Transaction lock helpers are unavailable."}
        )

    try:
        amount_int = sanitize_positive_int(amount, "amount", max_value=1_000_000)
        currency = normalize_currency(currency)
    except ValueError as e:
        return JSONResponse({"success": False, "msg": str(e)})

    txid = await acquire_txn_lock(transaction_id, email, f"admin_balance_{action}")
    if not txid:
        return JSONResponse(
    ...
    if action == "add":
        ...
        if res.modified_count != 1:
            await finish_txn_lock(txid, "failed", "user_not_found")
        ...
        if res.modified_count != 1:
            await finish_txn_lock(txid, "failed", "insufficient_or_not_found")
            return JSONResponse(
                {
                    "success": False,
        ...
        await finish_txn_lock(txid, "failed", "invalid_action")
        return JSONResponse({"success": False, "msg": "Invalid action"})

    await finish_txn_lock(txid, "done", "ok")
    return JSONResponse(
        {
            "success": True,
...
async def change_password(
    ...
    if not user.get("password"):
        return JSONResponse(
            {
                "success": False,
                "msg": "Your account uses Google/Telegram login — no password to change.",
            }
        )
...
async def change_email_verify(request: Request, code: str = Form(...)):
    ...
    otp_doc = await db.otps.find_one(
        {"email": current_email, "code": code, "type": "email_change"}
    )
    if not otp_doc:
        return JSONResponse({"success": False, "msg": "Invalid verification code."})
    if (datetime.now() - otp_doc["created_at"]).total_seconds() > 300:
    ...
    _set_store_session_cookie(response, request, new_email)
    return response
...
async def admin_email_verify(
    ...
    otp_doc = await db.otps.find_one(
        {"email": email, "code": code, "type": "admin_email_change"}
    )
    if not otp_doc:
...
async def get_my_tickets(request: Request):
    ...
    tickets = (
        await db.support_tickets.find({"email": email})
        .sort("created_at", -1)
        .to_list(50)
    )
    for t in tickets:
...
async def admin_change_ticket_status(
    ...
    result = await db.support_tickets.update_one(
        {"_id": ticket_id}, {"$set": {"status": status}}
    )
