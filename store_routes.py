...
async def store_manage_balance(
    ...
    if not all([callable(sanitize_positive_int), callable(normalize_currency), callable(acquire_txn_lock), callable(finish_txn_lock)]):
        return JSONResponse({"success": False, "msg": "Balance tools are unavailable. Please contact admin."})
