...
async def public_storefront(request: Request):
    try:
        ...
        if not currencies: currencies = [{"_id": "EGP", "symbol": "EGP"}, {"_id": "USD", "symbol": "USD"}]
        ...
        return templates.TemplateResponse("storefront.html", {
            "request": request, "categories": categories, "currencies": currencies,
            "stock": stock_details, "client_id": GOOGLE_CLIENT_ID, "maintenance": maintenance
        })
...
def get_user_data(user):
    ...
    for key, val in user.items():
        if key.startswith("balance_"): data[key] = val
    return data
...
async def signup_request(request: Request, name: str = Form(...), username: str = Form(...), email: str = Form(...), password: str = Form(...)):
    ...
    await db.otps.update_one({"email": email}, {"$set": {"code": code, "name": name, "username": username.lower(), "password": hash_password(password), "type": "signup", "created_at": datetime.now()}}, upsert=True)
...
async def google_login(request: Request, credential: str = Form(...)):
    async with httpx.AsyncClient() as client_http:
        ...
        if not user:
            user_id = await generate_unique_id()
            user = {"user_id": user_id, "username": email.split("@")[0].lower() + str(random.randint(10,99)), "email": email, "name": name, "balance_egp": 0, "balance_usd": 0, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M")}
...
async def telegram_login(request: Request, tg_id: str = Form(...), name: str = Form(...), username: str = Form("")):
    ...
    if not user:
        user_id = await generate_unique_id()
        user = {"user_id": user_id, "username": username.lower() if username else f"user{tg_id}", "email": email, "name": name, "balance_egp": 0, "balance_usd": 0, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M")}
        await db.store_customers.insert_one(user)
    response = JSONResponse({"success": True, **get_user_data(user)})
    response.set_cookie(key="store_session", value=email, httponly=True, max_age=86400 * 30)
    return response
...
async def customer_buy_uc(request: Request, stock_key: str = Form(...), price: float = Form(...), currency: str = Form(...)):
    ...
    if user.get(bal_field, 0) < price: return JSONResponse({"success": False, "msg": f"Insufficient {currency.upper()} balance! Please recharge."})
...
async def get_my_orders(request: Request):
    ...
    for o in orders: o['order_id'] = str(o['_id'])
...
async def get_profile(request: Request):
    ...
    return JSONResponse({"success": True, "user": {"name": user.get("name", ""), "email": user.get("email", ""), "avatar": user.get("avatar", ""), "created_at": user.get("created_at", "")}})
