...
async def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📜 السجلات والبحث الشامل", callback_data="admin_logs_hub")],
        [InlineKeyboardButton("🏠 خروج", callback_data="back_home")]
    ])

def admin_logs_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔍 البحث الشامل الذكي", callback_data="admin_global_search")],
...
# 🆕 قائمة التحكم المدمجة مع شاشة عرض المستخدمين
def admin_users_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚙️ تعديل رتبة / إدارة توكنات", callback_data="admin_search_manage_user")],
        [InlineKeyboardButton("📜 سجلات مستخدم معين", callback_data="admin_get_user_logs_btn")],
        [InlineKeyboardButton("🔙 رجوع للأدمن", callback_data="admin_panel")]
    ])

def stock_manage_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📥 الرفع الذكي للملفات", callback_data="admin_choose_cat_file"), InlineKeyboardButton("✍️ إضافة يدوي", callback_data="admin_choose_cat_manual")],
...
def success_pull_keyboard(callback_data):
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 سحب فئة أخرى", callback_data="pull_stock_menu"), InlineKeyboardButton("🔄 نفس الفئة", callback_data=callback_data)], [InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_home")]])

def retry_keyboard(callback_data, back_to="back_home"):
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 مرة أخرى", callback_data=callback_data)], [InlineKeyboardButton("🔙 رجوع", callback_data=back_to)]])
...
def admin_users_back_btn(): return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع لقائمة المستخدمين", callback_data="admin_users_menu")]])
