...
async function ensureStoreSession(force = false) {
    const hasLocal = !!localStorage.getItem('store_email') && !!localStorage.getItem('store_name');
    if (!force && _serverAuthKnown && _serverLoggedIn) return true;
    if (!force && !hasLocal && _serverAuthKnown && !_serverLoggedIn) return false;

    const ok = await fetchAndApplyProfile();
    if (!ok) _clearAuthLocals();
    return ok;
}
...
async function openProfileModal() {
    ...
    const hasLocal = !!localStorage.getItem('store_email') && !!localStorage.getItem('store_name');
    if (!hasLocal || !_serverAuthKnown) await ensureStoreSession(true);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const isLoggedIn = await ensureStoreSession(true);
